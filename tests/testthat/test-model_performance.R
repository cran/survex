test_that("model_performance works", {
    ...
    expect_lte(cph_rot_perf$`C-index`, 1)
    expect_gte(cph_rot_perf$`C-index`, 0)
    expect_lte(cph_rot_perf$`Integrated C/D AUC`, 1)
    expect_gte(cph_rot_perf$`Integrated C/D AUC`, 0)
    expect_lte(cph_rot_perf$`Integrated Brier score`, 1)
    expect_gte(cph_rot_perf$`Integrated Brier score`, 0)
    expect_equal(length(cph_rot_perf$`Brier score`), length(cph_exp_rot$times))
    expect_equal(length(cph_rot_perf$`C/D AUC`), length(cph_exp_rot$times))
    expect_true(all(cph_rot_perf$eval_times == cph_exp_rot$times))

    expect_lte(rsf_rot_perf$`C-index`, 1)
    expect_gte(rsf_rot_perf$`C-index`, 0)
    expect_lte(rsf_rot_perf$`Integrated C/D AUC`, 1)
    expect_gte(rsf_rot_perf$`Integrated C/D AUC`, 0)
    expect_lte(rsf_rot_perf$`Integrated Brier score`, 1)
    expect_gte(rsf_rot_perf$`Integrated Brier score`, 0)
    expect_equal(length(rsf_rot_perf$`Brier score`), length(rsf_exp_rot$times))
    expect_equal(length(rsf_rot_perf$`C/D AUC`), length(rsf_exp_rot$times))
